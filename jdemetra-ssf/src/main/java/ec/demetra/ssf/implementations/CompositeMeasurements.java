...
public class CompositeMeasurements implements ISsfMeasurements {
     ...
     * H = R R' = D L L' D, so that R = D * L (each column of L is multiplied by 
    ...
    public double ZVZ(int pos, int v, int w, SubMatrix V) {
        SubMatrix Vvw = V.extract(cdim[v], cdim[v + 1], cdim[w], cdim[w + 1]);
        if (v == w) {
    ...
    public void VpZdZ(int pos, int v, int w, SubMatrix V, double d) {
        ...
        if (v == w) {
            ...
            do {
                double cur = zw.get(cols.getPosition());
                if (cur != 0) {
                    measurements[v].XpZd(pos, col, cur * d);
