 ...
 * @author Jean Palate
 */
public class LevenbergMarquardtMinimzer implements ISsqFunctionMinimizer {
    ...
    private final double tau = DEF_INIT_MU;
    ...
    public boolean minimize(ISsqFunction fn, IReadDataBlock pstart) {
        return minimize(fn.ssqEvaluate(pstart));
    }
    ...
    public ISsqFunctionMinimizer exemplar() {
        LevenbergMarquardtMinimzer ex = new LevenbergMarquardtMinimzer();
        ex.eps1 = eps1;
        ex.eps2 = eps2;
        ex.eps3 = eps3;
        ex.eps2_sq = eps2_sq;
    ...
    public Matrix getCurvature() {
        if (V == null) {
            int n = fn_.getDomain().getDim();
            ISsqFunctionDerivatives derivatives = fcur_.getSsqDerivatives();
            V = Matrix.square(n);
            derivatives.getHessian(V.all());
    ...
    protected boolean iterate() {
        ...
        fcur_.getSsqDerivatives().getJacobian(J.all());
