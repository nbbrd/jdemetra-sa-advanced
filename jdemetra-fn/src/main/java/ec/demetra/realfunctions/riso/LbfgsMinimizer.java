 ...
 */
package ec.demetra.realfunctions.riso;
...
public class LbfgsMinimizer implements IFunctionMinimizer {
    ...
    public Matrix getCurvature() {
        ...
        m_fcur.getDerivatives().getHessian(H.all());
        return H;
    }

    @Override
    public IReadDataBlock getGradient() {
        return m_fcur.getDerivatives().getGradient();
    }
    ...
    public boolean minimize(IFunctionInstance start) {
        ...
        do {
            try {
                success = false;
                if (!failed) {
                    df = m_fcur.getDerivatives();
                }
                double[] grad = new double[n];
                df.getGradient().copyTo(grad, n);
                m_lbfgs.lbfgs(n, m_m, x, failed ? 2 * Math.abs(fcur) : fcur, grad, false, diag,
