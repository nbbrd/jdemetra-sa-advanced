...
public class MovingTradingDaysSaProcessor {

    public MovingTradingDaysSaResults process(TsData s, RegArimaSpecification regSpec,
            MovingTradingDaysSpecification mtdSpec, X11Specification xspec, ProcessingContext context) {
        return process(regSpec.build(context).process(s, null), mtdSpec, xspec);
    }

    public MovingTradingDaysSaResults process(TsData s, TramoSpecification regSpec,
            MovingTradingDaysSpecification mtdSpec, X11Specification xspec, ProcessingContext context) {
        return process(regSpec.build(context).process(s, null), mtdSpec, xspec);
    }

    private MovingTradingDaysSaResults process(PreprocessingModel model, MovingTradingDaysSpecification mtdSpec, X11Specification xspec) {
        MovingTradingDaysEstimator mtde = new MovingTradingDaysEstimator(mtdSpec);
        MovingTradingDaysPreprocessingFilter filter = new MovingTradingDaysPreprocessingFilter(mtde);
        ...
        if (filter.process(model)) {
            ...
            return MovingTradingDaysSaResults.builder()
                    .preprocessing(model)
                    .movingTradingDaysCorrection(mtdc)
                    .decomposition(rslts)
            ...
            return MovingTradingDaysSaResults.builder()
                    .preprocessing(model)
                    .movingTradingDaysCorrection(null)
                    .decomposition(rslts)
                    .finalDecomposition(finalStep(model, filter, rslts))
                    .mstatistics(Mstatistics.computeFromX11(spec.getMode(), rslts.getInformation()))
                    .build();
