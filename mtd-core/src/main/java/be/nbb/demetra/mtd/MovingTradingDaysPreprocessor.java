 ...
 * @author Jean Palate
 */
public class MovingTradingDaysPreprocessor implements
        IX11Preprocessor {

    private X11Context context;
    private final MovingTradingDaysPreprocessingFilter filter;
     ...
     * @param filter
     */
    public MovingTradingDaysPreprocessor(MovingTradingDaysPreprocessingFilter filter) {
        this.filter=filter;
    }
    ...
    public void preprocess(InformationSet info) {
        ...
        TsData td=filter.getMovingTradingDaysEstimator().getTdEffect();
        td=TsData.add(td, lp);
