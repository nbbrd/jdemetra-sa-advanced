...
//        System.out.println(Qq2);
    ...
    public void testSelection() {
        ...
        PeriodSelector sel = new PeriodSelector();
        sel.from(LocalDate.MIN);
        assertTrue(s.select(sel).isEmpty());
        sel.to(LocalDate.MAX);
        assertTrue(s.select(sel).isEmpty());
    ...
    public void testExtendTo() {
        ...
        LocalDate d0 = LocalDate.of(1980, Month.DECEMBER, 31);
        assertTrue(s.extendTo(d0).getLastPeriod().lastDay().equals(d0));
        LocalDate d1 = LocalDate.of(1980, Month.DECEMBER, 30);
        assertTrue(s.extendTo(d1).getLastPeriod().lastDay().isBefore(d1));
        LocalDate d2 = LocalDate.of(1981, Month.JANUARY, 2);
