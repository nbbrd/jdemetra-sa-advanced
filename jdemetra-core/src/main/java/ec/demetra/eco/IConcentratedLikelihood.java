...
public interface IConcentratedLikelihood extends ILikelihood {
     ...
     * @return
     */
    IReadDataBlock getCoefficients();
     ...
     * @return
     */
    default IReadDataBlock getCoefficientsStdev(boolean unbiased, int hpcount) {
        Matrix var = getCoefficientsCovariance();
        if (var == null) {
            return DataBlock.EMPTY;
        }
        DataBlock diag = var.diagonal().deepClone();
        if (unbiased) {
            double n = getN();
            double c = n / (n - diag.getLength() - hpcount);
     ...
     */
    default int getNx(){
        return getCoefficients().getLength();
    }
     ...
     * @return
     */
    default IReadDataBlock getTStats() {
     ...
     * @return
     */
    default IReadDataBlock getTStats(boolean unbiased, int hpcount) {
        Matrix var = getCoefficientsCovariance();
        if (var == null) {
            return null;
        }
        ...
        DataBlock t = new DataBlock(getCoefficients());

        t.apply((x, y) -> x == 0 ? 0 : x / Math.sqrt(c * y), var.diagonal());
        return t;
    }
