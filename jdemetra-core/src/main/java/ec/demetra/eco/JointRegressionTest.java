...
public class JointRegressionTest {
     ...
     * @return
     */
    public boolean accept(IConcentratedLikelihood ll, int nhp, int ireg, int nregs, InformationSet info) {
        final double f;
        if (nregs > 1) {
            Matrix bvar = new Matrix(ll.getCoefficientsCovariance(nhp >= 0, nhp).subMatrix(ireg, ireg + nregs, ireg, ireg + nregs));
            SymmetricMatrix.lcholesky(bvar);
            double[] b = new double[bvar.getRowsCount()];
            ll.getCoefficients().rextract(ireg, nregs).copyTo(b, 0);
