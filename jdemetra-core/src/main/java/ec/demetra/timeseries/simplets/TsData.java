...
public class TsData implements Cloneable, Iterable<TsObservation>, IReadDataBlock {
    ...
    public TsData extendTo(final LocalDate lastday) {
    ...
    public TsData lag(int nperiods) {
        TsPeriod s = getStart().minus(nperiods);
    ...
    public TsData lead(final int nperiods) {
        TsPeriod s = getStart().plus(nperiods);
        return new TsData(s, vals, true);
    }
     ...
     * returned.
     */
    public TsData select(final PeriodSelector ps) {
        if (ps == null) {
            return clone();
        }
