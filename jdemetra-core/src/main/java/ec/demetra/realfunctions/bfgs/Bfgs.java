...
package ec.demetra.realfunctions.bfgs;
...
public class Bfgs implements IFunctionMinimizer {
    ...
    private void calcGrad(double[] g) {
        IReadDataBlock grad = m_ftry.getDerivatives().getGradient();
        grad.copyTo(g, 0);
    }
    ...
    private boolean canStop(int test) {
        ...
        if (test == 1) {
            DataBlock x=new DataBlock(m_x), xprev=new DataBlock(m_xprev);
            return m_f <= m_fprev
                    && ((m_fprev - m_f) / Math.abs(m_fprev) < m_eps && x.distance(xprev)/(1+x.nrm2())<m_xeps);
        } else if (test == 2) {
    ...
    public Matrix getB() {
        ...
        if (!m_bUpdated) {
            ...
            if (m_bdirty < m_bminiter || !update()) {
                Matrix h = Matrix.square(m_fn.getDomain().getDim());
                new NumericalDerivatives(m_ftry, true).getHessian(h.all());
    ...
    public boolean minimize(IFunction function, IReadDataBlock pstart) {
        return minimize(function.evaluate(pstart));
    }
