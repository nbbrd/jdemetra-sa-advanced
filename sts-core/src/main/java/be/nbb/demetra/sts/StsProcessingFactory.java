...
public class StsProcessingFactory extends GenericSaProcessingFactory implements IProcessingFactory<StsSpecification, TsData, CompositeResults> {
    ...
    private static SequentialProcessing<TsData> create(StsSpecification xspec, ProcessingContext context) {
    ...
    protected StsProcessingFactory() {
    }

    public static CompositeResults process(TsData s, StsSpecification xspec) {
        SequentialProcessing<TsData> processing = create(xspec, null);
        return processing.process(s);
    }

    @Override
    public SequentialProcessing<TsData> generateProcessing(StsSpecification xspec, ProcessingContext context) {
        return create(xspec, context);
    }

    public SequentialProcessing<TsData> generateProcessing(StsSpecification xspec) {
    ...
    protected static <S extends IProcSpecification> IProcessingNode<TsData> createDeterministicStep() {
        return new IProcessingNode<TsData>() {
            ...
            public IProcessing.Status process(TsData input, Map<String, IProcResults> results) {
                ...
                StsEstimation estimation = estimation(results);
                if (estimation == null) {
                    return IProcessing.Status.Invalid;
                }
    ...
    protected static <S extends IProcSpecification> IProcessingNode<TsData> createDecompositionStep() {
        return new IProcessingNode<TsData>() {
            ...
            @Override
            public IProcessing.Status process(TsData input, Map<String, IProcResults> results) {
                StsEstimation e = estimation(results);
                if (e == null) {
                    return IProcessing.Status.Unprocessed;
                }
                ...
                StsDecomposition decomposition = new StsDecomposition(y, e.getModel(), mul);
    ...
    protected static StsEstimation estimation(Map<String, IProcResults> results) {
        IProcResults imodel = results.get(ESTIMATION);
        if (imodel == null || !(imodel instanceof StsEstimation)) {
            return null;
        }
        return (StsEstimation) imodel;
    }
