...
public class MixedAirlineResults implements ISaResults {
    ...
    private DefaultSmoothingResults srslts_;
    ...
    private void computeDecomposition(MixedAirlineModel model) {
        SsfData data = new SsfData(y_);
        ISsf ssf = MixedAirlineSsf.of2(model, y_.getStart().getPosition());
        srslts_ = DkToolkit.sqrtSmooth(ssf, data, true);
        noise_ = new TsData(y_.getStart(), srslts_.getComponent(ssf.getStateDim() - 1));
        enoise_ = new TsData(y_.getStart(), srslts_.getComponentVariance(ssf.getStateDim() - 1));
        enoise_.apply(x -> x < 0 ? 0 : Math.sqrt(x));
